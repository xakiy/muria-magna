...
class Orang_Schema(Skema):
    ...
    nik = fields.String(
        required=True, validate=Length(min=16, max=16, error="NIK harus 16 digit")
    )
    ...
    pendidikan_akhir = fields.Nested(
        "Pendidikan_Akhir_Schema", only=("nama", "singkatan"), allow_none=True
    )
    alamat = fields.Nested(
        "Alamat_Schema", only=("jenis", "alamat", "default"), many=True
    )
...
class Santri_Schema(Orang_Schema):
    ...
    nis = fields.String(
        required=True, validate=Length(min=16, max=16, error="NIS tidak tepat")
    )
...
class Pengguna_Schema(Skema):
    orang = fields.Nested(
        "Orang_Schema",
        only=("id", "nama", "jinshi", "tempat_lahir", "tanggal_lahir", "tanggal_masuk"),
        dump_to="profile",
    )
    ...
    kewenangan = fields.Nested(
        "Kewenangan_Schema", only=("wewenang"), dump_to="wewenang"
    )
